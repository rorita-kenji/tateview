...
function init() {
  ...
  // 未読状態のとき、原稿表示領域のクリックで FSA ファイル選択ダイアログを開く
...
/**
 * FSA (showOpenFilePicker) でファイルを開く。
 * fallbackFile が渡され、FSA が未対応またはキャンセルされた場合は File から直接開く（handle なし）。
 */
async function openFileViaFsa(opts = {}) {
  const fallbackFile = opts.fallbackFile || null;
  const keepPosition = !!opts.keepPosition;
  ...
  if (window.showOpenFilePicker) {
    try {
      ...
      const file = await handle.getFile();
      await openFile(file, handle, { keepPosition });
      return;
    } catch { /* キャンセル or エラー */ }
  }

  // FSA 未対応 → fallback File から開く（自動更新なし）
  if (fallbackFile) {
    await openFile(fallbackFile, null, { keepPosition });
  } else {
    $('fileInput').click();
  }
}
...
/* ---------------- ページ捲り（横スワイプ / トラックパッド横スクロール） ----------------
 * カルーセル方式。#pageStrip を指に 1:1 で追従させ、隣ページをスライドインさせる。
 *
 * 1スワイプ＝1ページ。1回のジェスチャー中に送れるのは1ページだけで、
 * 確定後の残りの指の動き・慣性イベントは捨てる（pageSwipeTaken）。
 * ただしジェスチャーの切れ目では待たせない: 次の指が触れた瞬間／次の横スクロールが
 * 始まった瞬間に、進行中の着地アニメを即確定して受け付ける（連続で捲れる）。
 */
let pageFitScale = 1;
/** ストリップ横オフセット px（右=+ ＝次ページ方向） */
let pageSwipeTx = 0;
/** ドラッグ開始時のストリップ位置（途中から引き継ぐ用） */
let pageSwipeBase = 0;
/** このジェスチャーで既に1ページ送ったか（1スワイプ＝1ページ） */
let pageSwipeTaken = false;
/** 指／ホイールで追従中 */
let pageSwipeActive = false;
/** 着地アニメ中（入力は受けるが render は抑制） */
...
let wheelAccum = 0;
let persistTimer = 0;
/** 入力が止まってからハイライト再描画・字級フィットをやり直すまでの時間 */
const SETTLE_QUIET_MS = 140;
let settleTimer = 0;
/* 横ホイールの慣性判別用。macOS の慣性イベントは |deltaX| が単調に減衰するので、
   立ち上がり（増加）や逆向きを見れば「次のスワイプ」だと分かる。 */
let wheelLastT = 0;
let wheelLastDir = 0;
let wheelPrevAbs = 0;
/** 直近にホイールでページを送った時刻 */
let wheelLastCommitT = -1e9;
/** 間隔が詰まって送りきれなかったぶんの予約（符号つき枚数） */
let wheelQueue = 0;
let wheelQueueTimer = 0;
/** 慣性が尽きた（次の有効な入力から新ジェスチャー） */
let wheelEnded = false;


/** 指を離した瞬間の判定。少しでも横に払っていれば送る（入力を捨てない） */
const SWIPE_COMMIT_RATIO = 0.04;
const SWIPE_COMMIT_MIN_PX = 20;
/** フリック判定（px/ms）。ほとんど動かなくても勢いがあれば送る */
const SWIPE_FLICK_V = 0.15;
/** 「シュパパパ」を成立させるため着地は短く */
const SWIPE_ANIM_MS = 150;
/** 横ホイール（Mac トラックパッドの2本指スワイプ）の増幅率 */
const WHEEL_GAIN = 1.8;
/** 横ホイールがこの時間途切れたら1ジェスチャー終了とみなす */
const WHEEL_IDLE_MS = 110;
/** この間隔が空いたら別のスワイプ（慣性イベントは 16ms 前後で連続して来る） */
const WHEEL_GAP_MS = 60;
/** 新しいスワイプと認めるのに要る deltaX。慣性の尻尾を拾わないための下限 */
const WHEEL_NEW_MIN_PX = 4;
/** これ未満は「ほぼ止まっている」。慣性が尽きた／指が止まった目印 */
const WHEEL_STOP_PX = 2;
/** ホイールでページを送る最短間隔（誤検出の保険。連打は妨げない長さ） */
const WHEEL_MIN_PAGE_MS = 100;
/** 横ホイールはこれだけずれたら即確定（1スワイプ1ページなので軽くてよい） */
const WHEEL_COMMIT_RATIO = 0.06;
const WHEEL_COMMIT_MIN_PX = 30;
/** ドラッグでここまで引き切ったら指を離す前に確定 */
const SWIPE_FULL_RATIO = 0.98;

/* --- 診断モード: URL の末尾に #swipedebug を付けると、
       スワイプが実機でどう届いているかを画面に出す。通常は完全に無効。 --- */
const SWIPE_DEBUG = typeof location !== 'undefined' && /swipedebug/.test(location.hash || '');
let swipeLogEl = null;
let swipeLogLines = [];
/** @param {string} msg */
function swipeLog(msg) {
  if (!SWIPE_DEBUG) return;
  if (!swipeLogEl) {
    swipeLogEl = document.createElement('div');
    swipeLogEl.id = 'swipeDebug';
    document.body.appendChild(swipeLogEl);
  }
  swipeLogLines.push(msg);
  if (swipeLogLines.length > 14) swipeLogLines.shift();
  swipeLogEl.textContent = swipeLogLines.join('\n');
}

/** 生の入力列（切り分け用に1行で残す） */
let swipeRawLine = [];
function swipeRaw(dx, gap, isNew) {
  if (!SWIPE_DEBUG) return;
  swipeRawLine.push(`${dx > 0 ? '+' : '-'}${Math.abs(dx).toFixed(0)}` +
    (gap > 40 ? `_${gap.toFixed(0)}ms` : '') + (isNew ? '★' : ''));
  if (swipeRawLine.length > 60) swipeRawLine.shift();
  const el = document.getElementById('swipeRaw') || (() => {
    const d = document.createElement('div');
    d.id = 'swipeRaw';
    document.body.appendChild(d);
    return d;
  })();
  el.textContent = swipeRawLine.join(' ');
}

function slideWidth() {
  // スロットは #pageStrip の 100% 幅。pageWrap.clientWidth（padding 込み）だと
  // 満幅アニメが行き過ぎて、復位で「戻る」ように見える。
...
/** 連続捲り中の位置保存は間引く（localStorage 書き込みでカクつかせない） */
...
/**
 * アニメを待たずに 1ページ差し替える。
 * peer が用意済みならその DOM をそのまま本体に移すので再描画は1枚分で済む。
 * @param {1|-1} dir
 * @returns {boolean} 送れたか（端なら false）
 */
function stepPageInstant(dir) {
  const ni = clampPage(state.pageIndex + dir);
  if (ni === state.pageIndex) return false;
  const pageEl = $('page');
  const peerEl = $('pagePeer');
  state.pageIndex = ni;
  syncPendingFromPage();
  ...
  if (peerEl && pageEl && pageSwipePeerDir === dir &&
      pageSwipePeerIndex === ni && peerEl.childNodes.length) {
    pageEl.className = peerEl.className.includes('grid') ? 'page-sheet grid' : 'page-sheet';
    pageEl.replaceChildren(...Array.from(peerEl.childNodes));
    peerEl.textContent = '';
    pageSwipePeerDir = 0;
    pageSwipePeerIndex = -1;
  } else if (pageEl) {
    paintPageSheet(pageEl, ni, { highlights: false });
  }
...
/**
 * 追従の中心。1スワイプ＝1ページなので、確定済みならもう動かさない。
 * 1ページ分まで引き切ったら、指を離す前でもその場で確定する。
 * @param {number} x 望みのストリップ位置 px
 */
function trackSwipeTx(x) {
  if (!state.pages.length) return;
  // 確定済み／着地アニメ中: このジェスチャーの残りは捨てる
  if (pageSwipeTaken || pageSwipeLock) return;

  const w = slideWidth();
  let tx = x;

  // 端では引っぱっても付いてこない（ゴムバンド）
  const atStart = state.pageIndex <= 0;
  const atEnd = state.pageIndex >= state.pages.length - 1;
  if ((tx > 0 && atEnd) || (tx < 0 && atStart)) {
    const sgn = tx > 0 ? 1 : -1;
    tx = sgn * Math.min(w * 0.2, Math.abs(tx) * 0.3);
  }
  // 隣ページ1枚ぶんより先へは行かせない
  ...
  if (Math.abs(tx) > 1) ensureSwipePeer(/** @type {1|-1} */ (tx > 0 ? 1 : -1));
  else hideSwipePeer();

  applyStripTransform({ live: true });

  if (Math.abs(tx) >= w * SWIPE_FULL_RATIO) {
    const dir = /** @type {1|-1} */ (tx > 0 ? 1 : -1);
    if (clampPage(state.pageIndex + dir) !== state.pageIndex) commitSwipe(dir);
  }
}

/**
 * 新しいスワイプの開始。着地アニメが残っていれば待たずに確定させる。
 * これが無いと素早い連続スワイプが食われる。
 */
function beginSwipeGesture() {
  if (pageSwipeLock) settleSwipeNow();
  abortSwipeAnimation();
  pageSwipeTaken = false;
  if (wheelIdleTimer) {
...
/**
 * 着地アニメ。dir 方向へ1ページ分スライドしてから差し替える。
 * @param {1|-1} dir
 */
function commitSwipe(dir) {
  swipeLog(`commit ${dir > 0 ? '次' : '前'} p${state.pageIndex + 1}` +
    (pageSwipeTaken ? ' [済のため無視]' : ''));
  if (pageSwipeTaken || !ensureSwipePeer(dir)) {
    if (!pageSwipeTaken) springSwipeHome();
    return;
  }
  abortSwipeAnimation();
  // このジェスチャーで送れるのはここまで（残りの指の動き・慣性は捨てる）
  pageSwipeTaken = true;
  pageSwipeLock = true;
  pageSwipeActive = false;
...
function finishSwipeCommit(dir, gen) {
  ...
  settleCurrentPage(pageEl, strip);
}

/**
 * 捲り終わりの後始末: ハイライト付きで描き直して字級フィットを取り直す。
 * 連続捲り中は呼ばない（重いので着地時だけ）。
 */
function settleCurrentPage(pageEl, strip) {
  // 連打中は走らせない。ハイライト再描画＋実寸計測はページ1枚ぶんの
  // 強制レイアウトを伴うので、1捲りごとに実行すると入力が詰まる。
  if (settleTimer) clearTimeout(settleTimer);
  settleTimer = setTimeout(() => {
    settleTimer = 0;
    if (pageSwipeLock || pageSwipeActive || wheelQueue) {
      settleCurrentPage(pageEl, strip); // まだ動いている。落ち着いてから
      return;
    }
...
function springSwipeHome() {
  ...
  const done = () => {
    ...
    // 連続捲りの途中で戻した場合もページは進んでいるので、
    // ハイライトと字級フィットを取り直す。
    settleCurrentPage($('page'), strip);
  };
  /** @param {TransitionEvent} ev */
  const onEnd = (ev) => {
    if (ev.target !== strip) return;
...
/**
 * 着地アニメ中に次の入力が来たら、待たずに結果だけ確定して受け付ける。
 * （これが無いと素早い連打が食われる）
 */
function settleSwipeNow() {
  if (!pageSwipeLock) return;
  const dir = pageSwipeTx > 0 ? 1 : -1;
  abortSwipeAnimation();
  pageSwipeLock = false;
  finishSwipeCommit(/** @type {1|-1} */ (dir), pageSwipeGen);
}

/**
 * 指を離したとき: 移動量か勢いでページを送る／戻す。
 * @param {number} [velocity] px/ms（右方向が +）
 */
function onSwipeRelease(velocity) {
  if (pageSwipeLock) return;
  pageSwipeActive = false;
  if (pageSwipeTaken) return; // 1スワイプ1ページ: もう送らない
  const tx = pageSwipeTx;
  const v = velocity || 0;
  let dir = 0;
  if (Math.abs(tx) >= commitThreshold()) dir = tx > 0 ? 1 : -1;
  else if (Math.abs(v) >= SWIPE_FLICK_V && Math.abs(tx) > 4) dir = v > 0 ? 1 : -1;

  if (dir && clampPage(state.pageIndex + dir) !== state.pageIndex) {
    commitSwipe(/** @type {1|-1} */ (dir));
    return;
  }
  if (Math.abs(tx) > 1) springSwipeHome();
  else {
    hideSwipePeer();
    resetPageStrip(true);
  }
...
/**
 * この wheel イベントが「新しいスワイプの開始」か。
 *
 * Chrome は「指の動きか慣性か」を JS に教えてくれない。慣性は途切れずに
 * 流れ続けるので、速度の変化から推測すると 1スクロールで複数ページ進む
 * 誤検出が避けられない。そこで推測はやめ、誤検出しない事実だけを使う:
 *   - イベントの流れが途切れた
 *   - 逆向きに振った
 *   - ほぼ停止（慣性が尽きた）あとに、また動き出した
 * これ以外は、どれだけ強く払われても同じ1スワイプの続きとみなす。
 * @param {number} dx deltaX
 * @param {number} t timeStamp
 */
function isNewWheelSwipe(dx, t) {
  const abs = Math.abs(dx);
  const dir = dx > 0 ? 1 : -1;
  if (!wheelLastT) return true;
  const gap = t - wheelLastT;

  // ほぼ停止 = このジェスチャーは終わり。次のまともな入力から数え直す
  if (abs < WHEEL_STOP_PX) {
    wheelEnded = true;
    wheelLastDir = 0;
    return false;
  }
  if (abs < WHEEL_NEW_MIN_PX) {
    // 慣性の尻尾。間が空いていたら前のジェスチャーは終わったとみなす
    if (gap > WHEEL_GAP_MS) {
      wheelEnded = true;
      wheelLastDir = 0;
    }
  ...
  if (gap > WHEEL_GAP_MS) return true;                   // 流れが途切れた
  if (wheelEnded) return true;                           // 止まったあと動き出した
  if (wheelLastDir && dir !== wheelLastDir) return true; // 逆向きに振った
  return false;
}

function wheelCommitThreshold() {
  return Math.max(WHEEL_COMMIT_MIN_PX, slideWidth() * WHEEL_COMMIT_RATIO);
}

function onWheel(e) {
  // Mac トラックパッドの2本指横スワイプ。1スワイプ＝1ページ。
  // 慣性は捨てるが、慣性の途中に来た「次のスワイプ」は必ず拾う（＝シュパパパ）。
  if (Math.abs(e.deltaX) > Math.abs(e.deltaY) && Math.abs(e.deltaX) > 0.5) {
    ...
    const dx = e.deltaX;
    const isNew = isNewWheelSwipe(dx, t);
    swipeRaw(dx, wheelLastT ? t - wheelLastT : 0, isNew);
    swipeLog(`w dx=${dx.toFixed(1)} gap=${wheelLastT ? (t - wheelLastT).toFixed(0) : '-'}` +
      `${wheelEnded ? ' 停止後' : ''}${isNew ? ' ★NEW' : (pageSwipeTaken ? ' (捨)' : '')}`);
    const abs = Math.abs(dx);
    if (isNew) {
      beginSwipeGesture();
      wheelEnded = false;
    ...
    wheelPrevAbs = abs;
    wheelLastT = t;
    // 向きは「意味のある大きさの入力」だけで更新する。
    // 慣性の尻尾で上書きすると、逆向きへの振り直しを取りこぼす。
    if (abs >= WHEEL_NEW_MIN_PX) wheelLastDir = dx > 0 ? 1 : -1;

    trackSwipeTx(pageSwipeTx - dx * WHEEL_GAIN);

    // 指を離すイベントが無いので、少しずれた時点で確定してしまう
    if (!pageSwipeTaken && Math.abs(pageSwipeTx) >= wheelCommitThreshold()) {
      const dir = /** @type {1|-1} */ (pageSwipeTx > 0 ? 1 : -1);
      if (t - wheelLastCommitT < WHEEL_MIN_PAGE_MS) {
        // 送った直後。捨てずに予約して、間隔が空いてから必ず送る
        swipeLog('queue');
        pageSwipeTaken = true;
        pageSwipeTx = 0;
        applyStripTransform({ live: true });
        queueWheelPage(dir, WHEEL_MIN_PAGE_MS - (t - wheelLastCommitT));
      } else if (clampPage(state.pageIndex + dir) !== state.pageIndex) {
        wheelLastCommitT = t;
        commitSwipe(dir);
      }
    }
  ...
  // 縦ホイール（マウス）: 一定量ごとに1ページ
  e.preventDefault();
  if (pageSwipeActive || pageSwipeLock) return;
  wheelAccum += e.deltaY;
...
/**
 * 送る間隔が詰まりすぎたぶんを予約する。入力は捨てない。
 * @param {1|-1} dir
 * @param {number} delay ms
 */
function queueWheelPage(dir, delay) {
  wheelQueue = Math.max(-3, Math.min(3, wheelQueue + dir));
  if (wheelQueueTimer) return;
  wheelQueueTimer = setTimeout(function drain() {
    wheelQueueTimer = 0;
    if (!wheelQueue) return;
    const d = /** @type {1|-1} */ (wheelQueue > 0 ? 1 : -1);
    wheelQueue -= d;
    if (pageSwipeLock) settleSwipeNow();
    pageSwipeTaken = false;
    pageSwipeTx = 0;
    // イベントの timeStamp と同じ時間軸で進める
    wheelLastCommitT += WHEEL_MIN_PAGE_MS;
    swipeLog(`queued ${d > 0 ? '次' : '前'}`);
    if (clampPage(state.pageIndex + d) !== state.pageIndex) commitSwipe(d);
    else wheelQueue = 0;
    if (wheelQueue) wheelQueueTimer = setTimeout(drain, WHEEL_MIN_PAGE_MS);
  }, Math.max(10, delay));
}

/** 横スクロールが途切れた＝1ジェスチャー終了 */
function endWheelGesture() {
  wheelIdleTimer = 0;
  if (!pageSwipeTaken && !pageSwipeLock) onSwipeRelease(0);
  pageSwipeTaken = false;
  wheelLastT = 0;
  wheelLastDir = 0;
  wheelPrevAbs = 0;
...
/**
 * タッチ／ペンの横スワイプ。1フリック＝1ページ。着地を待たずに次を受け付ける。
...
function installSwipe(el) {
  ...
  const onDown = (e) => {
    ...
    // 前の指がまだ残っていても、新しい指は必ず拾う。
    // 連続スワイプでは pointerup が届く前に次の指が触れる。
    if (pointerId != null && pointerId !== e.pointerId) {
      if (!pageSwipeTaken && Math.abs(pageSwipeTx) > 1) onSwipeRelease(0);
      dropGesture();
    }
    // 着地アニメ中でも待たせない
    swipeLog(`down id=${e.pointerId} ${e.pointerType}`);
    beginSwipeGesture();
  ...
  const onMove = (e) => {
    ...
    if (pageSwipeTaken) return; // 1スワイプ1ページ: 確定後は指を追わない

    const t = e.timeStamp || performance.now();
    const dt = t - lastT;
    if (dt > 0) {
      const inst = (e.clientX - lastX) / dt;
      // 直近を重めに（速い短フリックは 2〜3 サンプルしか取れない）
      velocity = velocity * 0.35 + inst * 0.65;
      lastX = e.clientX;
      lastT = t;
    }
    trackSwipeTx(pageSwipeBase + dx);
  };
  ...
  const onUp = (e) => {
    ...
    // ドラッグ全体の平均速度も見る（サンプルが少ない高速フリックの取りこぼし防止）
    const span = lastT - t0;
    const mean = span > 0 ? (lastX - x0) / span : 0;
    const v = Math.abs(mean) > Math.abs(velocity) ? mean : velocity;
    swipeLog(`up id=${e && e.pointerId} tx=${pageSwipeTx.toFixed(0)} v=${v.toFixed(2)}` +
      `${pageSwipeTaken ? ' 済' : ''}`);
    dropGesture();
    // 指を離した瞬間に確定
    onSwipeRelease(idle > 120 ? 0 : v);
  };

  // pointer capture は使わない。
  // iOS では capture の解放時に飛ぶ lostpointercapture が「次の指」の
  // ジェスチャーを巻き添えで消してしまい、連続スワイプが効かなくなる。
  // タッチは既定で暗黙キャプチャされるので capture は元々不要。
  el.addEventListener('pointerdown', onDown, { passive: true });
  // move / up / cancel は指が要素外へ出ても確実に受けるため window で拾う
  // （pointermove は bubble するので el には付けない＝二重発火しない）
  window.addEventListener('pointermove', onMove, { passive: false });
  window.addEventListener('pointerup', onUp);
  window.addEventListener('pointercancel', (e) => {
    swipeLog(`cancel id=${e.pointerId}`);
    onUp(e);
  });
  // タブが隠れる等で pointerup を取り逃したときの保険
  window.addEventListener('blur', () => {
    if (pointerId == null) return;
    dropGesture();
    if (!pageSwipeLock && !pageSwipeTaken && Math.abs(pageSwipeTx) > 1) springSwipeHome();
  });
}
...
function bindUI() {
  $('fileInput').addEventListener('change', async (e) => {
    ...
    e.target.value = '';
    if (f) {
      await openFileViaFsa({ fallbackFile: f, keepPosition: reloadPick });
    }
  });
