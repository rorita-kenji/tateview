...
const SEVERITY = {
  ...
  halfwidth: 'info',
  'fullwidth-alpha': 'info',
  'fullwidth-digit': 'info',
  emoji: 'info',
  'ruby-off': 'info',
  tab: 'warning',
...
export function detectWarnings(text, opt = {}) {
  ...
  scan(text, /[Ａ-Ｚａ-ｚ]+/g, (m) => {
    ...
    add('fullwidth-alpha', m.index, m.index + m[0].length);
  });
  scan(text, /[０-９]+/g, (m) => {
    if (!on('fullwidth-digit') || excluded(m.index)) return;
    add('fullwidth-digit', m.index, m.index + m[0].length);
  });

  // 絵文字（info）。ZWJ結合・肌色・国旗・キーキャップを1シーケンスとし、連続はラン1件。
  // ルビ内・見出しプレフィックスは除外。※や★など通常の記号は対象外（Extended_Pictographic 中心）。
  if (on('emoji')) {
    scanEmojiRuns(text, (start, end) => {
      if (!excluded(start)) add('emoji', start, end);
    });
  }
...
/**
 * 絵文字シーケンスを走査し、隣接する連続を1ランにまとめて cb(start, end)。
 * - Extended_Pictographic + 任意 VS16 + 肌色 + タグ列 + ZWJ 連鎖
 * - 国旗（地域指示子×2）
 * - キーキャップ（数字/#/* + 任意 VS16 + U+20E3）
 */
function scanEmojiRuns(text, cb) {
  // u フラグ必須。数字単体の \p{Emoji} は拾わない（Extended_Pictographic 中心）。
  const re =
    /(?:\p{Regional_Indicator}{2})|(?:[0-9#*]\uFE0F?\u20E3)|(?:\p{Extended_Pictographic}\uFE0F?(?:[\u{1F3FB}-\u{1F3FF}])?(?:[\u{E0020}-\u{E007E}]+\u{E007F})?(?:\u200D\p{Extended_Pictographic}\uFE0F?(?:[\u{1F3FB}-\u{1F3FF}])?)*)/gu;
  let runStart = -1;
  let runEnd = -1;
  let m;
  re.lastIndex = 0;
  ...
  if (runStart >= 0) cb(runStart, runEnd);
}
