...
> **本プロジェクトは実装済み（FIX）。最終の確定事項は末尾「## 実装確定メモ（v3 / FIX → v1.2）」を参照し、以前の記述と差異がある場合はそちらを優先すること。**
...
- 左サイド（幅 **112px**；`max-width: 1024px` では **56px**＝半分）：**章／話**記号（`#markBar`・サムネ上）＋**ミニマップサムネ**（全ページ・Yスクラブ・章/話色バー・「Nページ」）＋controls。チェック **2列×3行**（短ラベル: 禁則・ぶら下／ルビ・半角／空白・升目）。テーマ・書体・字級。クレジット `v1.3` / by やまもりやもり。狭幅では ◀▶・クレジット・文字数行を非表示、チェック1列、章話は入力のみ。
