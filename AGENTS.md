# AGENTS.md — 縦view（TateView）実装指示書 v3（実装確定）／v1.3
...
> **本プロジェクトは実装済み（FIX）。最終の確定事項は末尾「## 実装確定メモ（v3 / FIX → v1.3）」を参照し、以前の記述と差異がある場合はそちらを優先すること。**
> プロダクト名：**縦view**、バージョン：v1.3、配布ファイル：**dist/TateView.html**。
...
プロダクト名：**縦view**、バージョン **v1.3**、配布：**dist/TateView.html**。
...
- 左サイド（幅 **112px**；`max-width: 1024px` では **56px**＝半分）：［**見出し**］（`#headBar`・サムネ上）＋**ミニマップサムネ**（全ページ・Yスクラブ・章/話色バー・「Nページ」）＋controls。チェック **2列×3行**（短ラベル: 禁則・ぶら下／ルビ・半角／空白・升目）。テーマ・書体・字級。クレジット `v1.3` / by やまもりやもり。狭幅ではクレジット・文字数行を非表示、チェック1列。ページ◀▶はサイドに無し（本文左右 edge）。
...
### PWA（novedit 準拠・network-first）

- 成果物: `dist/TateView.html` + `dist/manifest.webmanifest` + `dist/sw.js` + `dist/icons/*`（build.js が同梱）
- head: https は実ファイル manifest／file:// は data URL。SW は https/localhost/127.0.0.1 のみ登録
- SW: `tateview-shell-v1` network-first。オンラインで shell 更新、オフラインは前回キャッシュ。版チェック UI なし
- `file_handlers` + `launchQueue` で第1 .txt を openFile(handle)
- mypage: `scripts/publish-web.sh` の tateview が PWA 一式をコピー

### v1.3（PWA・見出しダイアログ・左右 edge）

- 製品バージョン **v1.3**（`package.json` 1.3.0、UI クレジット `v1.3`）。
- **PWA**: `manifest.webmanifest` + `sw.js`（`tateview-shell-vN` network-first）+ `icons/`（濃灰 `#424242` + 白「縦」）。`build.js` が `dist/` に同梱。publish は HTML だけでなく一式。
- **見出し**: `#headBar`［見出し］primary → `#overlayHead`。`headCfg`（novedit 既定記号・builtin KW）。旧 `#markBar` 廃止。warnings/search に `headCfg` を渡す。
- **ページ送り**: `#pageEdgeNext`（左=次）`#pageEdgePrev`（右=前）。サイド ◀▶ 廃止。本文優先フィット。`edges-roomy` 塗り / `edges-tight` 枠線透明三角（薄め opacity）。
- **字級**: `contentAvailSize` は左右帯を引かない。`measureExtent` は scale 解除後に計測。maxFit ±1px ヒステリシス。gap は fs=100 換算。

### ページ捲り（v1.2 で確定）
### ページ捲り（横スワイプ／横スクロール）

**1スワイプ＝1ページ。** 1回のジェスチャーで送るのは必ず1ページで、行き過ぎない。

- **タッチ／ペン**：指を離した瞬間に確定する（20px 払うか 0.15px/ms の勢いがあれば送り、足りなければ元へ戻す）。1ページ幅まで引き切った場合は離す前に確定。着地アニメの完了は待たず、次に指が触れた瞬間に新しいスワイプとして受け付ける。前の指の `pointerup` が届く前に次の指が触れても取りこぼさない。
- ポインタキャプチャは使わない。`lostpointercapture` が次のジェスチャーを巻き添えで破棄するため。`pointermove` / `pointerup` / `pointercancel` は `window` で受ける。
- **トラックパッド等の横スクロール（wheel）**：ブラウザは「指の動きか慣性か」を JS に知らせない。速度パターンからの推測は 1スクロールで複数ページ進む誤検出と表裏一体なので、推測はしない。次のスワイプの合図は**誤検出しない事実のみ**とする。
  - イベントの流れが 60ms 以上途切れた
  - 逆向きに振った
  - ほぼ停止（|deltaX| < 2px）したあと、また動き出した
  慣性が途切れないまま同方向に重ねた連打は 1ページにまとめる（行き過ぎ厳禁を優先）。
- ページ送りが詰まった場合、入力は捨てずに予約して間隔（100ms）が空いてから送る。
- 連続で捲る操作中は、ハイライト再描画と字級フィットの再計算を入力が 140ms 止まるまで遅延させる（強制レイアウトで入力を詰まらせないため）。
- 診断：URL に `#swipedebug` を付けて開くと、入力の届き方と生の `deltaX` 列を画面に表示する。
