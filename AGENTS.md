# AGENTS.md — 縦view（TateView）実装指示書 v3（実装確定）／v1.01
...
> プロダクト名：**縦view**、バージョン：v1.01、配布ファイル：**dist/TateView.html**。
...
プロダクト名：**縦view**、バージョン **v1.01**、配布：**dist/TateView.html**。
...
- 左サイド（幅 168px）：**章記号／話記号**（`#markBar`・サムネ上に常時1行）＋サムネ＋controls。チェックは **2列×3行**（禁則・ぶら下げ／ルビ・半角色／空白・升目）。テーマ。フォントと字級は横並び（「書体」ラベルなし）。クレジット `v1.01` / by やまもりやもり。
