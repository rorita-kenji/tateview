# AGENTS.md — 縦view（TateView）実装指示書 v3（実装確定）／v1.1
...
> **本プロジェクトは実装済み（FIX）。最終の確定事項は末尾「## 実装確定メモ（v3 / FIX → v1.1）」を参照し、以前の記述と差異がある場合はそちらを優先すること。**
> プロダクト名：**縦view**、バージョン：v1.1、配布ファイル：**dist/TateView.html**。
...
プロダクト名：**縦view**、バージョン **v1.1**、配布：**dist/TateView.html**。
...
- 左サイド（幅 168px）：**章記号／話記号**（`#markBar`・サムネ上に常時1行）＋**ミニマップサムネ**（領域内に全ページ表示・ドラッグで移動・章/話色バー・現在位置に「Nページ」）＋controls。チェックは **2列×3行**（禁則・ぶら下げ／ルビ・半角色／空白・升目）。テーマ。フォントと字級は横並び（「書体」ラベルなし）。クレジット `v1.1` / by やまもりやもり。
