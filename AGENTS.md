...
- 左サイド（幅 152px）：**章記号／話記号**（`#markBar`・サムネ上に常時1行）＋**ミニマップサムネ**（領域内に全ページ表示・ドラッグで移動・章/話色バー・現在位置に「Nページ」）＋controls。チェックは **2列×3行**（禁則・ぶら下げ／ルビ・半角色／空白・升目）。テーマ。フォントと字級は横並び（「書体」ラベルなし）。クレジット `v1.1` / by やまもりやもり。
