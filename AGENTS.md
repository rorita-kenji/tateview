# AGENTS.md — 縦view（TateView）実装指示書 v3（実装確定）／v1.2
...
> プロダクト名：**縦view**、バージョン：v1.11、配布ファイル：**dist/TateView.html**。
...
プロダクト名：**縦view**、バージョン **v1.11**、配布：**dist/TateView.html**。
...
- 左サイド（幅 **112px**；`max-width: 1024px` では **56px**＝半分）：**章／話**記号（`#markBar`・サムネ上）＋**ミニマップサムネ**（全ページ・Yスクラブ・章/話色バー・「Nページ」）＋controls。チェック **2列×3行**（短ラベル: 禁則・ぶら下／ルビ・半角／空白・升目）。テーマ・書体・字級。クレジット `v1.2` / by やまもりやもり。狭幅では ◀▶・クレジット・文字数行を非表示、チェック1列、章話は入力のみ。
